...
import { InventoryToggleButton } from "@/components/ui/inventory-toggle-button";
...
export function InventoryMenu({
  ...
  if (!isOpen) {
    return (
      <InventoryToggleButton onClick={() => setIsOpen(true)} isOpen={false} />
    );
  }

  return (
    <div className="flex flex-col items-end">
      <InventoryToggleButton onClick={() => setIsOpen(false)} isOpen={true} />
      <div className="w-full max-w-md mx-auto font-['Press_Start_2P',system-ui] text-[0.8rem] mt-6">
        <Card className="border-4 gap-y-0 w-[384px] h-[450px] py-0 border-[#352879] bg-[#0f0f1b] shadow-[0_0_0_4px_#000000,5px_5px_0_0_#000] rounded-none overflow-hidden">
          ...
          <Tabs
            ...
            className="w-full grow flex flex-col mt-4"
          >
            ...
            >
              <CardContent className="p-4 bg-[url('/placeholder.svg?height=100&width=100')] bg-repeat h-[300px] overflow-y-auto no-scrollbar">
                <TabsContent
                  value="items"
                  ref={itemsRef}
                  className="mt-0 space-y-4 transition-all duration-300 animate-in fade-in-50 pb-22"
                >
                  {items.map((item) => (
                    <div
                      key={item.id}
